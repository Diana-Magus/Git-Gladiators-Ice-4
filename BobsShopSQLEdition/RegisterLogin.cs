        ...
        public static bool checkUserDetailsRegister(User tempUser) //checks if user name exists
        {
            ...
            try
            {
                using (StreamReader sr = new StreamReader(path))
                {
                    ...
                    if (!Regex.IsMatch(tempUser.UserPassword, @"[0-9]+")) // Password needs to require a Number (StackOverflow. 20/01/2016.) (ByteHide. 21/05/2023.)
                    {
                        MessageBox.Show("Password needs to contain at least one number");
                    }
                    if (!Regex.IsMatch(tempUser.UserPassword, @"[A-Z]+")) // Password needs an Upper Character (StackOverflow. 20/01/2016.) (ByteHide. 21/05/2023.)
                    {
                        MessageBox.Show("Password needs to contain at least one uppercase character");
                    }
                    if (!Regex.IsMatch(tempUser.UserPassword, @".{8,}")) // Password cannot be no less than 8 Characters (StackOverflow. 20/01/2016.) (ByteHide. 21/05/2023.)
                    {
                        MessageBox.Show("Password needs to be at least 8 characters long");
                    }
                    if (Regex.IsMatch(tempUser.UserPassword, @"[0-9]+") && Regex.IsMatch(tempUser.UserPassword, @"[A-Z]+") && Regex.IsMatch(tempUser.UserPassword, @".{8,}")) //All requirements are met (ByteHide. 21/05/2023.)
                    {
                        return true;
                    }
                }
            ...
            MessageBox.Show("Username not available, please enter a different one");
            return false;
        }
